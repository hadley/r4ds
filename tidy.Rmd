...
```{r ref.label = "tidy-pivot-longer-plot-lines"}
```
...
Suppose you'd like to calculate the `case_rate` (number of `cases` divided by `population`) for each country in a given year, and record it as a new column, resulting in the following data frame.

```{r tidy-pivot-longer-case-ratio, echo = FALSE}
table2 %>%
...
```{r tidy-pivot-wider, echo = FALSE, out.width = "100%", fig.cap = "Pivoting `table2` into a \"wider\", tidy form."}
...
```{r ref.label = "tidy-pivot-longer-case-ratio"}
```

Suppose you wanted to go one step further and widen the data to record `cases` and `population` for 1999 and 2000 in separate columns.
...
```{r tidy-separate, echo = FALSE, out.width = "75%", fig.cap = "Separating `rate` into `cases` and `population` to make `table3` tidy"}
...
Table \@ref(tab:who1-keys) provides the list of all `key`s in this dataset.

```{r who1-keys, echo = FALSE}
who1 %>% 
  distinct(key) %>%
  mutate(key_new = str_replace(key, "newrel", "new_rel")) %>%
  separate(key_new, into = c("new", "type", "sexage"), sep = "_", remove = FALSE) %>%
  arrange(type, sexage) %>%
  select(key, type) %>%
  group_by(type) %>%
  mutate(id = 1:n()) %>%
  pivot_wider(id_cols = id, names_from = type, values_from = key) %>%
  select(rel, ep, sn, sp) %>%
  knitr::kable(
    col.names = c("", "", "", ""),
    caption = "All `key`s in the `who1` data frame."
    )
```
...
3.  The next letter gives the sex of TB patients. The dataset groups 
...
We need to make a minor fix to the format of the column names: unfortunately the names are slightly inconsistent because instead of `new_rel` we have `newrel`. You'll learn about `str_replace()` in [strings], but the basic idea is pretty simple: replace the characters "newrel" with "new_rel". This makes all variable names consistent.
...
```{r}
who6 %>%
  filter(var == "sp", country == "United States of America") %>%
  group_by(year) %>%
