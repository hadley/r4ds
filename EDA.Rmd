...
In creating this bar chart, we mapped the variable we want to divide the data into first to the `x` aesthetic and the variable we then further want to divide each group into to the `fill` aesthetic.

```{r}
#| fig.alt: >
#|   A bar chart of cuts of diamonds, sehmented by color. The number of diamonds 
#|   for each level of cut increases from Fair to Ideal, and there are some 
#|   diamonds with each color in each level of cut.
