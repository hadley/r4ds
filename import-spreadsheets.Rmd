...
This is useful within excel because it reduces duplication... but it's also verty useful when identifying when cells to read...

### Data types

In the underlying for CSV files, all the values are strings.
This is not particularly true to the data, but it is simple: everything is a string.

The underlying data in spreadsheets is more complex.
...
Note, importantly that there's no special type for date/times --- in a spreadsheet date/times are just numbers with special formatting.

Something about floating point numbers --- in my sheet it looks like 9 but in R it looks like 8.99999.
Jenny will know more.

So what happens if you have a column that's a mix of these types?
readxl has to make some decisions.

Confusingly it's also possible to have something that looks like a number but is actually a string (e.g. type `'10` into excel).

`col_type = "list"` keeps the "original" values without trying to get all the info out.
...
(Need some comment about code being not of the tidyverse. And you'll need to install separately. Camel case; can't pipe; argument in different order. Feel unfamiliar. But that's ok; you'll encounter this as you expand outside of this book)
...
Needs some brief discussion of formatting --- lots of people want to fully control how the sheet looks.
Just need some pointers where to look.
