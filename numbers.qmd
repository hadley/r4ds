...
And you've computed the time between each events, and figured out if there's a gap that's big enough to qualify:
...
`cumsum()`, from @sec-cumulative-and-rolling-aggregates, comes to the rescue as gap, i.e. `has_gap` is `TRUE`, will increments `group` by one (@sec-numeric-summaries-of-logicals):
