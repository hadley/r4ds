...
You can perform the same computation "by hand" with `summarise()` and `n()`.
...
2.  Expand the following calls to `count()` to instead use `summarise()` and `arrange()`:
...
4.  What does `flights |> filter(row_number() < 4, .by = dest)` do?
    What does `flights |> filter(row_number(dep_delay) < 4, .by = dest)` do?
...
flights |> 
  mutate(
    .by = year:day,
    r = min_rank(desc(sched_dep_time))
  ) |> 
