...
seattle_lib_folder <- "data/seattle-library-checkouts"
seattle_lib_files <- tibble(
  files = list.files(seattle_lib_folder, recursive = TRUE),
  size = file.size(file.path(seattle_lib_folder, files))
)
seattle_lib_files
```
...
seattle_lib2 <- open_dataset(seattle_lib_folder)
```
