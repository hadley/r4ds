...
There's also an even more direct connection between arrow and dbplyr -- it's very easy to turn an arrow dataset into a duckdb datasource.
This doesn't involve any memory copying, and speaks to the goals of the arrow ecosystem: enable seamless transitions from one computing framework to another.
This can be done using the `to_duckdb()` function supplied by the arrow package:
...
In this example, all the statements that appear after `to_duckdb()` are executed using the duckdb engine, and there is an analogous function `to_arrow()` you can use to pass control back from duckdb to arrow.
...
When this structuring is done intelligently, the multi-file strategy can lead to significant improvements in performance because only a subset of the files need to be read in order to analyze the data.
