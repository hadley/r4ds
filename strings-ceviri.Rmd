...
Bu bölüm R'da dizileri işlemeyi tanıtıyor. Dizilerin nasıl çalıştığını ve el ile nasıl oluşturulduğunu öğreneceksin, ama bölümün asıl odağı kurallı ifadeler (regex) olacak. Diziler genelde .... bilgilerden oluştuğu için kurallı ifadeler bu dizilerdeki bu ifadeleri .... tanımlamakta kullanışlıdır. Bir kurallı ifadeye ilk bakışında bir kedinin klavyen üstünde yürüdüğünü düşüneceksin ama bilgin geliştikçe ifadeler kısa zamanda anlamlı gelmeye başlayacak.      
...
Bu bölüm __stringr__ paketi ile dizileri manüpile etmeye odaklanacak. Her zaman text verin olmayacağı için stringr tidyverse'in asıl kısımlarından değil, bu yüzden stringr ekstra olarak yüklememiz gerekiyor.
...
Dizileri tek tırnak ya da çift tırnak ile oluşturabilirsin. Diğer dillerin aksine, hangisiyle oluşturduğun bir fark yaratmaz. Eğer birden fazla `"` içeren bir dizi oluşturmayacaksan, ben her zaman `"` kullanmanı öneririm.
...
Eğer bir tırnağı kapatmayı unututursan devam ettirme işareti olan `+` yı göreceksin: 
...
Eğer böyle birşey görürsen, Escape tuşuna bas ve tekrar dene! 

<!-- To include a literal single or double quote in a string you can use `\` to "escape" it:-->

Bir dizide gerçek bir tek ya da çift tırnak kullanmak için "kaçış" işaretini (`\`) kullanabilirsin:
...
Bu demek oluyor ki eğer diziye gerçek bir sola eğik çizgi eklemek istersen, onu iki kere yazman gerek: `"\\"`.
...
Dikkat edilmelidir ki bir dizinin basılı ifadesi dizinin kendisiyle aynı değildir çünkü basılı ifade kaçış işaretlereini de gösterir. Dizinin ham içeriğini görmek için `writeLines()` kullan:
...
Pekçok farklı özel karakter de vardır. Bunların en yaygınları `"\n"`, yeni satır ve `"\t"`, tab'dır, ancak tüm listeyi `"`: `?'"'`, ya da `?"'"`  üzerinde yardım isteyerek görebilirsin. Bazen de her platformda çalışan İngilizce olmayan karakterlerin bir gösterimi olan,`"\u00b5"` gibi diziler görebilirsin:
...
Birden fazla dizi genellikle `c()` ile oluşturabileceğin bir karakter vektöründe tutulur:
...
Temel R, dizilerle çalışmak için pekçok farklı fonksiyon içerir ama bunlardan kaçınmalıyız çünkü tutarsız olabiliyorlar ve bu da hatırlanmalarını zorlaştırıyor. Bunun yerine stringr fonksiyonlarını kullanacağız. Bu fonksiyonların daha sezgisel isimleri var ve hepsi `str_` ile başlıyor. Örneğin, `str_length()` sana bir dizideki karakter sayısını verir:
...
Eğer RStudio kullanıyorsan bu yaygın `str_` ön eki bilhassa kullanışlıdır, çünkü `str_` yazmak bütün stringr fonksiyonlarını görmeni sağlayan otomatik tamamlamayı tetikler:
...
İki ya da daha fazla diziyi birleştirmek için `str_c()` kullan: 
...
Nasıl ayrılacaklarını kontrol etmek için `sep` argümanını kullan:
...
R'daki diğer birçok fonksiyon gibi kayıp değerler bulaşıcıdır. Eğer onları `"NA"` olarak yazdırmak istiyorsan `str_replace_na()` kullan:
...
Yukarda görüldüğü gibi str_c()` vektörleştirilmiştir ve kısa vektörleri otomatik olarak en uzunla aynı boya dönüştürür:
...
Boyu 0 olan objeler sessizce çıkartılır. Bu özellikle `if` bağlaçlarında kullanışlıdır:
...
Dizilerden oluşan bir vektörü tek bir diziye daraltmak için `collapse` kullan:
...
<!--### Subsetting strings-->
###Dizilerede alt kümeleme

<!--You can extract parts of a string using `str_sub()`. As well as the string, `str_sub()` takes `start` and `end` arguments which give the (inclusive) position of the substring:-->

bura üstünde çalışmak lazım---`str_sub()` ile bir dizinin partlarını elde edebilirsin. Dizi gibi, `str_sub()` alt dizinin (kapsayan) pozisyonlarını başlangıç (`start`) ve bitiş (`end`) argumanları
...
Dikkate alınmalı ki `str_sub()` dizi çok kısa olsa bile başarısız olmayacaktır: sadece mümkün olanı döndürecektir:
...
Dizileri modifiye etmek için `str_sub()`'ın atama yöntemini de kullanabilirsin:
...
Yukarıda metni küçük harfe çevirmek için `str_to_lower()` kullandım. Sen `str_to_upper()` ya da `str_to_title()` de kullanabilirsin. Yine de, harflerin durumunu değiştirmek ilk bakışta göründüğünden daha karmaşık olabilir çünkü  farklı dillerin farklı kuralları vardır. Sen bir lokal belirleme yaparak kurallar seti seçebilirsin:
