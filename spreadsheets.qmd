...
If you'd like to write to a specific sheet, you can specify that with the `sheet` argument as well.
...
For further authentication details, we recommend reading the documentation for `gs4_auth()`.

### Exercises

1.  Read the Google Sheet titled survey from <https://pos.it/r4ds-survey>, with `survey_id` as a character variable and `n_pets` as a numerical variable.

2.  Read the Google Sheet titled roster from <https://pos.it/r4ds-roster>. The resulting data frame should be called `roster` and should look like the following.
