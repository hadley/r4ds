...
The matches determines the rows in output, a new data frame that contains the key, the x values, and the y values.
For example, @fig-join-inner shows an inner join, where two rows match if and only if the keys are equal.
