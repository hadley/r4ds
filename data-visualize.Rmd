...
The colors reveal that many of the unusual points (with engine size greater than 5 litres and highway fuel efficiency greater than 20 miles per gallon) are two-seater cars.
...
The first argument of `facet_wrap()` is `facets` defines based on which variable to split the plot into facets.
The variable(s) that you pass to this argument should be discrete and should be wrapped in the `vars()` function.
The second variable, `nrow`, tells `facet_wrap()` how many rows the facets should span.
You can similarly define `ncol` for the number of columns as well.

```{r fig.alt = "Scatterplot of highway fuel efficiency and engine size of cars in ggplot2::mpg, faceted by class, with facets spanning two rows."}
ggplot(data = mpg) + 
  geom_point(mapping = aes(x = displ, y = hwy)) + 
...
You specify the variable defining faceting on the rows with the `rows` argument and on the columns with the `cols` argument.
Variables passed to these arguments should also be wrapped in `vars()`.

```{r fig.alt = "Scatterplot of highway fuel efficiency versus engine size of cars in ggplot2::mpg, faceted by number of cylinders across rows and by type of drive train across columns. This results in a 4x3 grid of 12 facets. Some of these facets have no obvservations: 5 cylinders and 4 wheel drive, 4 or 5 cylinders and front wheel drive."}
ggplot(data = mpg) + 
  geom_point(mapping = aes(x = displ, y = hwy)) + 
  facet_grid(rows = vars(cyl), cols = vars(drv))
```

If you prefer to not facet in the rows or columns dimension, you can omit the respective argument `rows` or `cols`.

### Exercises

1.  What happens if you facet on a continuous variable?

2.  What do the empty cells in plot with `facet_grid(rows = vars(cyl), cols = vars(drv))` mean?
    How do they relate to this plot?

    ```{r, eval = FALSE, fig.alt = "Scatterplot of number of cycles versus type of drive train of cars in ggplot2::mpg. Shows that there are no cars with 5 cylinders that are 4 wheel drive or with 4 or 5 cylinders that are front wheel drive."}
    ggplot(data = mpg) + 
...
3.  Recreate this plot using `facet_wrap()` instead of `facet_grid()`.
    ...
    ```{r eval = FALSE fig.alt = "Scatterplot of highway fuel efficiency versus engine size of cars in ggplot2::mpg, faceted by type of drive train across rows."}
    ggplot(data = mpg) + 
      geom_point(mapping = aes(x = displ, y = hwy)) +
...
4.  Take the first faceted plot in this section:
    ...
    ```{r, eval = FALSE, fig.alt = "Scatterplot of highway fuel efficiency and engine size of cars in ggplot2::mpg, faceted by class, with facets spanning two rows."}
    ggplot(data = mpg) + 
      geom_point(mapping = aes(x = displ, y = hwy)) + 
      facet_wrap(facets = vars(class), nrow = 2)
    ```
...
```{r, eval = FALSE, fig.alt = "Scatterplot of highway fuel efficiency versus engine size of cars in ggplot2::mpg with a smooth curve overlaid. A confidence interval around the smooth curves is also displayed."}
ggplot(data = mpg, mapping = aes(x = displ, y = hwy)) + 
  geom_point() + 
  geom_smooth()
```
...
```{r, message = FALSE, fig.alt = "Scatterplot of highway fuel efficiency versus engine size of cars in ggplot2::mpg, where points are colored according to the car class. A smooth curve following the trajectory of the relationship between highway fuel efficiency and engine size of cars is overlaid along with a confidence interval around it."}
ggplot(data = mpg, mapping = aes(x = displ, y = hwy)) + 
...
```{r, message = FALSE, fig.alt = "Scatterplot of highway fuel efficiency versus engine size of cars in ggplot2::mpg, where points are colored according to the car class. A smooth curve following the trajectory of the relationship between highway fuel efficiency and engine size of subcompact cars is overlaid along with a confidence interval around it."}
ggplot(data = mpg, mapping = aes(x = displ, y = hwy)) + 
  geom_point(mapping = aes(color = class)) + 
...
-   `position = "fill"` works like stacking, but makes each set of stacked bars the same height.
    ...
    ```{r fig.alt = "Segmented bar chart of cut of diamonds in ggplot2::diamonds, where each bar is filled with colors for the levels of clarity. Height of each bar is 1 and heights of the colored degments are proportional to the proportion of diamonds with a given clarity level within a given cut level."}
    ggplot(data = diamonds) + 
