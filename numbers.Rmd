...
It's impractical to list them all so this section will show the most useful.
...
It usually (but not always) gives you warning if the longer vector isn't a multiple of the shorter:
...
If the data is discrete, there may be multiple most common values, and if the data is continuous, there might be no most common value because every value is slightly different.
...
Sometimes you're not so interested in where the bulk of the data lies, but how it spreads out.
...
But the code below reveals it looks like one airport, [EGE](https://en.wikipedia.org/wiki/Eagle_County_Regional_Airport), might have moved.
...
You can do this with the base R `[` function, but we'll not cover it until Section \@ref(vector-subsetting), because it's a very powerful and general function.
...
However, because of the recycling rules they can also be usefully paired with `mutate()`, particularly when you want do some sort of group standardization.
For example:
