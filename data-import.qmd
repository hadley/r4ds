...
This heuristic works well if you have a clean dataset, but in real life you'll encounter all sorts of odd problems.
The following sections cover some of the most common issues you're like to encounter in practice, starting with non-standard missing values, .
...
```{r}
df <- read_csv(sample_data, col_types = list(x = col_double()))
```
