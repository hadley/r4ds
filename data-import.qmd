...
It's possible for a csv file to have column names that are not valid R variable names, names that are **non-syntactic**.
...
There are two useful functions to help you do this which different in whether you layout the tibble by columns or by row.
...
In this chapter, you've learned how to load load CSV files with `read_csv()` and to do your own data entry with `tibble()` and `tribble()`.
