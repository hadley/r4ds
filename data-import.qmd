...
In this very small case, you can easily see that the missing value `.`.
But what happens if you have thousands of rows with only a few speckled amongst them?
...
So when we set `na = "."`, the automatic guessing succeeds, giving us the numeric column that we want:
...
You might want to set `col_guess(.default = col_skip())` if you only want to read in the columns you specify:
