...
In many cases, the choice is made for you, as if the data is already in a database or in parquet files, you'll want to work with it as is.
But if you're starting with your own data (perhaps CSV files), you can either load into a database or convert to parquet.
...
In this chapter, we'll continue to use the tidyverse, particularly dplyr, but now we'll pair it with the arrow package which is designed specifically for working with large data.
...
We being by getting a dataset worthy of these tools: a data set of item checkouts from Seattle public libraries, available online at [data.seattle.gov/Community/Checkouts-by-Title/tmmm-ytt6](https://data.seattle.gov/Community/Checkouts-by-Title/tmmm-ytt6).
...
So we want to avoid `read_csv()`, and instead use the `open_dataset()` function from the arrow package:
...
The first line in the output tell you that `seattle_csv` is stored locally on-disk as a single csv file; it will only be loaded into memory when necessary.
The remainder of the output tell you the column type that arrow has imputed for each column.

We can see what's actually in the data by making arrow do some work with `glimpse()`.
This tells us that we have \~41 million rows and 12 columns, and shows some of the early values.
...
This highlights a useful feature of arrow: this code will work regardless of how large the underlying dataset is.
But it is currently rather slow: on Hadley's computer, the code took about 10s.
That's not terrible given how much data we have, but we can make it much faster by switching to a better format.

## The parquet format

To make this data easier to work with, we're going to do two things: we're going to switch to the parquet file format and we're going to split it up into multiple files.
