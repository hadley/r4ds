...
-   Select all columns except those from year to day (inclusive):
    ...
    Historically this operation was done with `-` instead of `!`, so you're likely to see that in the wild. These two operators serve the same purpose but with subtle differences in behaviour. We recommend to consistently use `!` because it reads as "not" and combines well with `&` and `|`.
