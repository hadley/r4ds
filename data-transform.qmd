...
In this section, you'll learn about the most important grouping function, `summarize()`, and well as how grouping can affect other variables.

### `summarize()` {#sec-summarize}

The most important grouped operation is a summary.
It's not very useful when you call it by itself:
...
It becomes powerful when you use the `.by` argument to specify how to divide the data into groups:
...
You can also use `.by` with `filter()` to express the same idea in a slightly different way:
...
But as you can see, `slice_max()` takes care of a lot of the details for you.

### Grouping by multiple variables

You can group by multiple variables with `c()`:
...
`.by` uses the same specification as `select()`, so you could also use `:`, `starts_with()`, `ends_with()`, or any other helper to select the variables:
...
It's also possible to use `.by` with `mutate()`.
This is particularly useful with the functions that you'll learn about in @sec-numbers, which allow you to do things like rank observations within a group:
...
The shape of this plot is very characteristic: whenever you plot a mean (or other summary) vs. group size, you'll see that the variation decreases as the sample size increases[^data-transform-3].

[^data-transform-3]: \*cough\* the central limit theorem \*cough\*.
...
The tools are roughly grouped into three categories: those that manipulate the rows (like `filter()` and `arrange()`, those that manipulate the columns (like `select()` and `mutate()`), and those that create summaries (like `summarise()`).
