...
You'll learn more about count in
...
-   Select all columns except those from year to day (inclusive):
    ...
    You can use `-` instead of `!` in the code above, but we recommend `!` to indicate "not" in a `select()` statement.
...
`group_by()` doesn't change the data but, if you look closely at the output, you'll notice that the output indicates that "grouped by" month (`Groups: month [12]`).
