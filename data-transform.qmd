...
Since the functionality of each verb is limited, solving complex problems will usually require combining multiple verbs, and we'll do so with the pipe, `|>`.
...
For example, this code shows the most delayed flights on top:
...
We'll come back to discuss missing values in detail in @sec-missing-values, but for now we'll tell the `mean()` function to ignore all missing values by adding the setting the argument `na.rm` to `TRUE`:
...
Now let's summarize see what happens when you summarize an ungrouped data frame.
