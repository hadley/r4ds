...
We recommend three of the five options: `fig-width`, `fig-asp` and `out-width`.
Set `fig-width` and `fig-asp` in the document YAML as below, and set `out-width` in individual chunks.
...
-   Control the output size with `out-width` and set it to a percentage of the body width of the output document.
    ...
    If you don't set `out-width`, the output width will be `fig-width`; if you set `out-width`, the output width will be expanded or shrunk from `fig-width` to the fixed width defined by `out-width`.

-   To put multiple plots in a single row, set the `layout-ncol` to 2 for two plots, 3 for three plots, etc.
    This effectively sets `out-width` to "50%" for two, "33%" for three, etc.
