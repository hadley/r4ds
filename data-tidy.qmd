...
This dataset collected by the WHO records information about tuberculosis diagnoses.
...
The first piece, `sp`/`rel`/`ep`, describes the method used for the diagnosis, the second piece, `m`/`f` is the `gender` (which is coded as a binary variable in this dataset), and the third piece, `014`/`1524`/`2535`/`3544`/`4554`/`65` is the `age` range (`014` represents 0-14, for example).
...
The core unit being studies is an organization, but each organization is spread across six rows, with one row for each measurement taken in the survey organization.
