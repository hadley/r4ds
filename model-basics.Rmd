...
It's also useful to see what the model doesn't capture, the so called residuals which are left after subsracting the predictions from the data. Residuals are powerful because they allow us to use models to remove striking patterns so we can study the subtler trends that remain.
...
Effectively, a model with a categorical `x` will predict the mean value for each category. (Why? Because the mean minimises the root-mean-squared distance.) That's easy to see if we overlay the predictions on top of the orignal data:
...
* Trees, e.g. `rpart::rpart()`, attack the problem in a complete different
  ...
  by models like random forrests (e.g. `randomForest::randomForest()`) or 
