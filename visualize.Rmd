...
This chapter focusses on ggplot2, one of the core components of the tidyverse. To access the datasets, help pages, and functions used in this chapter, load the tidyverse by running:
...
You can test your answer with the `mpg` __data frame__ found in ggplot2 (aka `ggplot2::mpg`). A data frame is a rectangular collection of variables (in the columns) and observations (in the rows). `mpg` contains observations collected by the US Environment Protection Agency on 38 car models. 
...
Similarly, we could have mapped `class` to the _alpha_ aesthetic, which controls the transparency of the points, or to the _shape_ aesthetic, which controls the shape of the points.
...
Here, the color doesn't convey information about a variable, but only changes the appearance of the plot. To set an aesthetic manually, set the aesthetic by name as an argument of your geom function. In other words, it goes _outside_ of `aes()`. You'll need to pick a value that makes sense for that aesthetic:
...
One way to add additional variables to a plot is by mapping them to an aesthetic. Another way, whic particularly useful for categorical variables, is to split your plot into __facets__, subplots that each display one subset of the data. 
...
* boxplots compute a robust summary of the distribution and display that 
  summary as a specially formatted box
...
This works because every geom has a default stat, and every stat has a default geom. This means that you can typically use geoms without worrying about the underlying statistical transformation. However, there are three occasions where you might need to use a stat explicitly:

1.  To override the default stat. The code below changes the stat of `geom_bar()`
    from count (the default) to identity. This maps the height of the bars to
    the raw values of a $y$ variable. Unfortunately, the term bar charts can
    be used to describe both this type of chart, where the height of the bar
    is already present in the data, or the previous bar chart, where the 
    ...
    able to guess their meaning from the context, and you'll learn exactly
    what they do soon!)

1.  To override the default mapping from transformed variables to aesthetics. 
	For example, to display a bar chart of proportions, rather than raw counts:
    ...
    To find the variables computed by a stat, look for the help section titled
    "computed variables".
    
1.  To draw greater attention to the statistical transformation in your code. 
	For example, you might use `stat_summary()`, which summarises the y values 
	for each unique x value, to draw attention to the computed summary:
...
ggplot2 provides more than 20 stats for you to use. Each stat is a function, so you can get help in usual way, e.g. `?stat_bin`. To see a complete list of stats, try the [ggplot2 cheatsheet](http://rstudio.com/cheatsheets).
...
The stacking is performed automatically using the __position adjustment__ specified by the `position` argument. If you don't want a stacked bar chart, you can use one of three other options: `"identity"`, `"dodge"` or `"fill"`.
...
The values of `hwy` and `displ` are rounded so the points appear on a grid, and many points overlap each other. This problem is known as __overplotting__. This arrangement makes it difficult to see the distribution of the data. Are the data points spread equally throughout the graph, or is there one special combination of `hwy` and `displ` that contains 109 values? 
...
In the previous sections, you learned much more than just how to make scatterplots, bar charts, and boxplots. You learned a foundation that you can use to make _any_ type of plot with ggplot2. To see this, let's add position adjustments, stats, coordinate systems, and faceting to our code template:
