...
-   `summarise()` plus `.by` computes a summary statistics for each subset.
...
df |> 
  summarise(across(everything(), median), .by = grp)
```
...
```{r}
summarise_means <- function(df, summary_vars = where(is.numeric), by = NULL) {
  df |> 
    summarise(
      .by = {{ by }},
...
diamonds |> 
  summarise_means(by = clarity)

diamonds |> 
  summarise_means(c(carat, x:z), by = clarity)
```
...
7.  Explain what each step of the pipeline in this function does.
    ...
    show_missing <- function(df, by, summary_vars = everything()) {
      df |> 
        summarise(
          .by = {{ by }},
