...
As well as computing the median without missing values, it'd be nice to know how many missing values there were.
...
In this section, you'll learn about `map()`'s friend `walk()`, which is designed to work with this sort of function.
...
Sometimes the database package will provide a handy function that will take a vector of paths and load them all into the database.
...
We're jumping the gun here a bit because you won't learn how to save a single plot until @sec-ggsave.
