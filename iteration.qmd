...
You've seen dplyr before, but purrr LINK?
is new.
...
The `.cols` argument uses the same specifications as `select()`, @sec-select, so you can use functions like `starts_with()` and `ends_with()` to select variables based on their name.
...
[^iteration-1]: Anonymous, because we never explicitly created it and gave it a name with `<-`.
    Another term programmers use for this is "lambda function".

    MAYBE: explain the older syntax `~` in case people see it?
...
df2 <- tibble(
  name = c("Amy", "Bob"),
  date = as.Date(c("2009-08-03", "2010-01-16"))
)

df2 |> 
  expand_dates()
# I think deserves to be applied to some actual data, i.e. complete the example. Most readers won't be able to imagine what the `list()` thing does. And I guess my immediate attempt to use it has failed?
```

`across()` also makes it easy to supply multiple variables in a single argument because the first argument uses tidy-select; you just need to remember to embrace that argument LINK.
...
Before we go on, it's worth pointing out an interesting connection between `across()` and `pivot_longer()` LINK.
...
# this feels unfinished, i.e. lacking the direct head-to-head comparison to something using across()
```
...
```{r}
# now **I** have inserted df2 as a suggestion for the lubridate example, but otherwise I don't understand why this is df3
...
6.  Adjust `expand_dates()` to automatically remove the date columns after they've been expanded.
    ...
    TODO: this callback is more reason why that example needs some more work

7.  Explain what each step of the pipeline in this function does.
    What special feature of `where()` are we taking advantage of?
    TODO: this chapter also depends on dev dplyr (pick()) but that's not called out as it is for purrr
...
There are three basic steps: use `dir()` to list all the files in a directory, then use `purrr::map()` to read each of them into a list, then use `purrr::list_rbind()` to combine them into a single data frame.
...
If your input data files change of over time, you might consider learning a tool like [targets](https://docs.ropensci.org/targets/) to set up your data cleaning code to automatically re-run whenever one of the input files is modified.
...
Now we can use `map()` to create a list of many plots[^iteration-5] and their eventual file paths:
