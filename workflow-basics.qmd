...
#TODO seems to me that you can create an object without assigning it, isn’t it?
...
#TODO the reader might think that at least, R could try to read her mind, which is feasible in this case (could find the closest string that makes sense or something like that). I’d rather write that R, like most programming langages, does not try to read your mind, even in cases it could try to with some chance of success, because that’s bound to be brittle; we (programmers) want the output to be, as surely as possible, the exact thing we meant, not the result of some obscure guesswork.
