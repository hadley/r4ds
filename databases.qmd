...
These datasets are easy to get into our learning database because dbplyr comes with a function copies the tables from nycflights13 to our database:
...
This is a one of the idiosyncrasies of SQL: `WHERE` is evaluated before `SELECT` and `GROUP BY`, it needs another clause that's evaluated afterwards.
...
The translation of summary functions becomes more complicated when you use them inside a `mutate()` because they have to turn into a so-called **window** function.
