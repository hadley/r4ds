...
In this chapter you'll also learn about a couple of ggplot2 extension packages you can use to access additional geometric objects not offered by ggplot2.
...
In @sec-geometric-objects you learned about `geom_point()` for making scatterplots and `geom_smooth()` for visualizing the trend in your data using a smooth curve.
...
For visualizing a single discrete (or categorical) variable, we can use `geom_bar()` to display the frequency of each value of that variable in the dataset.
Mapping the x aesthetic to the variable results in vertical bars while mapping the y aesthetic results in horizontal bars.

```{r}
#| layout-ncol: 2
#| fig-alt: |
#|   Bar plots showing the number of cars with different classes (SUV, 
#|   subcompact, pickup, minivan, midsize, compact, 2seater). The most 
#|   common cars are SUVs and then compacts, and the least common are 
#|   2seaters. On the plot on the left, class types are plotted along the 
#|   x-axis and the bars are vertical. On the plot on the right, class 
#|   types are plotted along the y-axis and the bars are horizontal.
...
While the choice of horizontal vs. vertical bar plots is entirely up to you, bar plots where the bars are horizontal are generally preferable if the labels along the x-axis overlap with each other.
For example, the following plots display some (arbitrary) values for US states with long names.
When the state names are on the x-axis, they overlap.
Placing them on the y-axis is one way to address this problem.

```{r}
#| layout-ncol: 2
#| echo: false
#| fig-alt: |
#|   Bar plots showing some states with long names (Massachusetts, 
#|   New Hampshire, North Carolina, North Dakota, Pennsylvania, 
#|   Rhode Island, South Carolina, South Dakota, West Virginia)
#|   On the plot on the left, states are plotted along the x-axis and 
#|   the bars are vertical. State names overlap due to their length and 
#|   the limited width of the plot. On the plot on the right, states 
#|   are plotted along the y-axis and the bars are horizontal.

df <- tibble(state = state.name) |>
  mutate(len = str_length(state)) |>
  filter(len >= max(len) - 2) |>
  mutate(value = round(runif(n(), min = 0, max = 100)))

ggplot(df, aes(x = state, y = value)) +
  geom_col()

ggplot(df, aes(x = value, y = state)) +
  geom_col()
```

In bar plots where the levels of the discrete variable being plotted are not ordered, like the car `class`es above, it's often preferable to reorder the bars based on their frequencies.
...
You will learn more about factors and functions for dealing with factors (like `fct_infreq()` shown below) in @sec-factors.
...
For a single continuous variable, we can use geoms like `geom_histogram()`, `geom_density()`, or `geom_freqpoly()`.
