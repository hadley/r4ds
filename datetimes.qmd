...
If you're reading a file with readr, you can choose to parse dates during input, by using `col_date()`, `col_time()`, or `col_datetime()` depending on whether you want a date, time, or date-time.

If your data contains an ISO8601 date-time, you don't need to do anything; readr will automatically recognize it.
...
If you're using `%b` or `%B` and working with non-English names, you'll also need to provide a `locale()`.
...
read_csv(
  csv,
  col_types = list(date = col_date("%d %B %Y")),
  locale = locale("fr")
)
```
