...
Otherwise, there are four ways you're likely to create a date/time:
...
If your data contains an ISO8601 date or date-time, you don't need to do anything; readr will automatically recognize it:
...
Year, month, and day are separated by `-`, hour, minute, and second are separated by `:` and the date and time components are separated by either a `T` or a space.

For other date/time formats, you'll need to use `col_types` plus `col_date()`, `col_datetime()`, or `col_time()`, along with a date-time formatting string.
This is a mini-language for describing dates that's used across many programming languages, mostly consisting of `%` followed by a single character.
...
This example shows some a few options applied to a very ambiguous date:
