...
But we strongly believe that it's best to master one tool at a time, and R is an great place to start.
...
RStudio is updated a couple of times a year, but RStudio will automatically let you know when a new version is out so there's no need to check back.
