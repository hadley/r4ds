...
This isn't quite the output we want, because it still includes all the list columns. This is default behaviour when `unnest()` works on single row data frames. To suppress these columns we use `.drop = TRUE`: (this argument has deprecated)
...
1.  What does this code do? Why might might it be useful?
    ...
    mtcars %>% 
      group_by(cyl) %>% 
      summarise_each(funs(list))  # This function has replaced
    ```
