...
1.  Usar columnas-lista (_list-columns_) para almacenar estructuras de datos arbitrarias en un _data frame_.
    Por ejemplo, esto te permitirá tener una columna que contenga modelos lineales.
...
1.  Usar el paquete __broom__, por David Robinson, para transformar modelos en datos ordenados.
    ...
    porque una vez que tienes datos ordenados, puedes aplicar todas las técncas que 
...
Este capítulo es de alguna forma aspiracional: si este libro es tu primera introducción a R, este capítulo es probable que sea una lucha para ti. Requiere que tengas profundamente internalizadas ideas acerca de modelización, estructuras de datos, e iteración. Así que no te preocupes si no lo entiendes --- solo aparta este capítulo por un par de meses, y vuelve cuando quieras ejercitar tu cerebro (TODO: ...when you want to stretch your brain). 

### Prerrequisitos

Trabajar con muchos modelos requiere muchos de los paquetes del __tidyverse__ (TODO: requires many of the packages of the tidyverse) (para exploración de datos, doma y programación) y __modelr__ para facilitar la modelización.
...
Separar facetas (TODO: Facetting) por continente es particularmente revelador:
...
Ahora que has visto un flujo de trabajo básico para manejar muchos modelos, vamos a sumergirnos en algunos detalles. En esta sección, exploraremos en más detalle la estructura de datos columna-lista. Solo recientemente es que he comenzado a apreciar realmente la idea de la columna-lista. Esta estructura está implícita en la definición de _data frame_: un _data frame_ es una lista nominada (TODO: o nombrada?) de vectores de igual largo. Una lista es un vector, así que siempre ha sido legítimo usar una lista como una columna de un _data frame_. Sin embargo, R base no hace las cosas fáciles para crear columnas-lista, y `data.frame()` trata a la lista como una lista de columnas:
...
(Si usas mucho este patrón, asegúrate de chequear `tidyr::separate_rows()` (del inglés _separar filas_) que es un _wrapper_ (TODO: función que resume varias funciones en una sola?) alrededor de este patrón común).
...
1.  ¿Qué hace este código? ¿Why might it be useful?
