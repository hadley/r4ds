...
We'll focus on three big topics: subsetting with `[`, subsetting with `[[` and `$` and iteration with for loops and the apply family of functions.
These tools are particularly useful as you being to do more programming in R, which is why we don't teach them earlier.

This is a good place to remind you that the tidyverse is not only way to solve data science problems.
We teach the tidyverse in this book because it's designed as whole, which tends to increase consistency across functions making it a little easier to use.
After you read this book we expect you'll learn other ways to tackle the same problems using base R, data.table, and other packages.
...
In this section, we'll introduce you to the power of `[`, first showing you how you can use it with vectors, then how the same principles extend in a straightforward way to 2d structures like data frames.
...
4.  **A character vector**.
    ...
    As with positive integers, you can use a character vector to duplicate individual entries.
...
We'll come back to `$` shortly, but we need to use it here because `[` doesn't use tidy evaluation, so you need to be explicit about the source of the `x` variable.

There's an important difference between tibbles and data frames when it comes to `[`.
If `df` is a data frame, `df[, cols]` will return a vector if `col` selects a single column and a data frame if it selects more than one column.
...
You can also avoid this problem by using 1d `[` to select columns:
...
-   `filter()` is equivalent to subsetting the rows with a logical vector, taking care to exclude missing values:
    ...
    ```{r}
    #| results: false
    df <- tibble(x = c(2, 3, 1, 1, NA), y = letters[1:5], z = runif(5))
    df |> filter(x > 1)
    ...
    Another common technique in the wild is to `which()` for it's side-effect of dropping missing values: `df[which(df$x > 1), ]`
...
-   `arrange()` is equivalent to subsetting the rows with an integer vector, usually created with `order()`.
    ...
    You can use the `decreasing` argument to sort all columns in decreasing order or `-rank(col)` to individual sort column in decreasing order.
...
[^base-r-2]: But it doesn't handle grouped data frames differently and it doesn't support the more selection helper functions like `starts_with()`.
...
1.  Create functions that take a vector as input and return:
    ...
    c.  Only even numbers (and no missing values).
...
`[`, which selects many elements, is paired with `[[` and `$`, which extract a single element from a list or data frame.
In this section, we'll show you how to use `[[` and `$` to pull columns out of a data frames, a couple of differences between `data.frames` and tibbles, and how `[` differs from `[[` when used with lists.
...
They can also be used to create new columns, the base equivalent of `mutate()`:
...
`[[` and `$` are also really important for working with lists, and it's really important to understand how they differ to `[`.
Lets illustrate the differences with a list named l:

```{r}
l <- list(a = 1:3, b = "a string", c = pi, d = list(-1, -5))
```
...
-   `[[` and `$` extract a single component from a list.
    They removes a level of hierarchy from the list.
...
The difference between `[` and `[[` is important for lists because `[[` drills down into the list while `[` returns a new, smaller list.
This difference very important, but it's easy to get confused.
To help you remember, take a look at the an unusual pepper shaker shown in @fig-pepper-1.
...
1.  What happens when you subset with a positive integer that's bigger than the length of the vector?
...
In @sec-iteration, you learned tidyverse techniques for iteration, primarily `dplyr::across()` and `purrr::map()`.
In this section, you'll learn about their base equivalents, first the apply family of functions and then the `for` loop, the powerful idea that underlies all these functions.

Some people will tell you to avoid `for` loops because they are slow.
They're wrong!
(Well at least they're rather out of date, as `for` loops haven't been slow for many years.) The chief benefit of using functions like `map()` or `apply()` is not speed, but clarity: once you've mastered the basic idea, they make your code faster to write and easier to read.

### Apply family

Another way of saying "map a function over each element of a vector" is "apply a function over each element of a vector", and base R provides a family of functions known as the "apply" family because they all use apply as a suffix.
...
In fact, because we don't use any of `map()`'s more advanced features, you can replace every `map()` call in @sec-iteration with `lapply()`.
...
There's no exact base R equivalent to `across()` but you can get close by using `[` twice along with `lapply():`
...
# Then transform each column with lapply() and replace the original values
...
It takes an additional argument that specifies the expected type which ensures that simplification occurs the same way regardless of the input.
For example, we could replace the `sapply()` call above with this `vapply()` where we specific that we expect `is.numeric()` to return a logical vector of length 1:
...
Unfortunately it returns its results in a named vector which requires some gymnastics if you want to collect multiple summaries into a data frame matched with the variable you're grouping by.
(It's certainly possible to avoid this and just work with free floating vectors, but that generally just makes more work down the road).
...
For loops are the fundamental building block of iteration that both the `apply()` and `map()` functions use under the hood.
For loops are powerful and general tool that you should definitely learn as you become a more experienced R programmer.
...
There are a few different techniques that you can use, but I prefer being explicit about what the output is going to look like upfront.
...
Rather than making a list and saving the results as we go, a simple approach is to build up the data frame piece-by-piece:
...
We recommend avoiding this pattern except for short experiments because growing the data frame in this way is a fundamentally slow approach.
