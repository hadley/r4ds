...
These are good to know about because you're likely to see in other people's code

-   `lapply()` = `map()`
-   `sapply()` = `lapply()` + `simplify`. Similar to `purrr::map_vec()`
-   `vapply()` = special. Similar to `map_int()` etc.
-   `tapply()` = used for single grouped summaries
-   `apply()` = used for applying functions to matrices by rows/columns.
