...
It's not possible to use the tidyverse without using base R, so we've already taught you a **lot** of base R functions: from `library()` to load packages, to `sum()` and `mean()` for numeric summaries, to the factor, date, and POSIXct data types, and of course all the basic operators like `+`, `-`, `/`, `*`, `|`, `&`, and `!`.
...
It's 100% okay to write code that uses a mixes of approaches, and don't let anyone tell you otherwise!
