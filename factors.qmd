...
Ordered factors, created with `ordered()`, imply a strict ordering between levels, but nothing about the magnitude of the differences.
You use them when you know that there is a ranking between the levels but the exact differences between levels is unknown.
You can recognize an ordered factor when printing because they use `<` between the factor levels:

```{r}
ordered(c("a", "b", "c"))
```

In the base R and the tidyverse, ordered factors behave very similarly to regular factors.
...
That means for the purposes of this book, correctly distinguishing between regular and ordered factors is not particular important.
More broadly however, certain fields (particularly the social sciences) do use them extensively, and thus many more specialised packages do provide different behaviours.
