...
Ordered factors, created with `ordered()`, imply a strict ordering between levels: the first level is "less than" the second level, the second level is "less than" the third level, and so on. This sounds like a numeric variable, but while the difference between 1 and 2 is the same as the difference between 2 and 3, for ordered factors the difference between levels is not defined, so it is employed when you know that there is a ranking between the levels but the exact differences between levels is unknown.
...
Ordered categories are frequently used in the social sciences but not so common in other fields, so unless you are faced with a dataset that has clearly ranked categories we don't generally recommend using ordered factors.
