...
6. Aggregate i birleştirme olarak kullandım. Toplam olarak çevriliyor sanırım ama bu sum()a karşılık gelecek ve kafa karışacak diye kullanmak istemedim. 
...
*   Aritmetik operatörler: `+`, `-`, `*`, `/`, `^`. Bunların hepsi "geri dönüşüm kuralları" kullanılarak vektörleştirilmiştir. Bir parametre diğerinden daha kısaysa, otomatik olarak aynı uzunlukta olacak şekilde genişletilir. Bu, bağımsız değişkenlerden biri tek bir sayı olduğunda yararlıdır: `air_time / 60`, `hours * 60 + minute`, etc.
    ...
    Aritmetik işleçler, daha sonra öğreneceğiniz birleştirme işlevleriyle birlikte de yararlıdır. Örneğin, `x / sum(x)` toplamın oranını hesaplar ve `y - mean(y)` ortalamadan farkı hesaplar.
...
Çok fazla sayıda eksik değer alırız! Bunun sebebi birleştirme fonksiyonlarının eksik değerlerin genel kuralına uymasıdır: eğer girdide eksik değer varsa, çıktı da eksik değer olur. Şanslıyız ki, tüm birleştirme fonksiyonları hesaplamadan önce eksik değerleri eleyen `na.rm` argümanına sahiptir.
