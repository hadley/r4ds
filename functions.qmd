...
```{r}
summary6 <- function(data, var, .by = NULL) {
  data |> summarise(
...
diamonds |> summary6(carat, .by = cut)
```

(Whenever you wrap `summarise()` in a helper, we think it's good practice to pass `.by` along so the user can choose the grouping.)
...
<!-- This section is totally invalidated -->
...
```{r}
#| error: true
count_missing <- function(df, x_var) {
  df |> 
    summarise(n_miss = sum(is.na({{ x_var }})), by = {{ by }})
}
flights |> 
