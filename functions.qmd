...
We'll start by looking at "mutate" functions, i.e. functions that work like `mutate()` and `filter()` because they return an output of the same length as the input.
...
Another important family of vector functions are summary functions, functions that return a single value for use in `summarize()`.
...
Or you maybe you want to make it easy to draw a bar plot just for a subset of the data:
