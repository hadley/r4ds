...
Data science is an exciting discipline that allows you to transform raw data into understanding, insight, and knowledge. The goal of "R for Data Science" is to help you learn the most important tools in R that will allow you to do data science efficiently and reproducibly. After reading this book, you will have the tools to tackle a wide variety of data science challenges, using the best parts of R. 

## What you will learn

Data science is a huge field, and there's no way you can master it all by reading a single book. Instead, our goal is to give you a solid foundation in the most important tools, and enough knowledge to find the resources to learn more when necessary. Our model of the tools needed in a typical data science project looks something like this:
...
Surrounding all these tools is __programming__. Programming is a cross-cutting tool that you use in nearly every part of a data science project. You don't need to be an expert programmer to be a successful data scientist, but learning more about programming generally pays off, because becoming a better programmer allows you to automate common tasks, and solve new problems with greater ease.
...
* Starting with data import and tidying is sub-optimal because 80% of the time, 
  these tasks are routine and boring, and the other 20% of the time, they're weird
  and frustrating. That's a bad place to start learning a new subject! Instead, 
  ...
  motivation will stay high because you know the frustration is worth the effort.
...
* Programming tools are not necessarily interesting in their own right, 
  but they do allow you to tackle considerably more challenging problems. We'll
  ...
  a wide range of interesting modelling problems.

Within each chapter, we try and adhere to a similar pattern: start with some motivating examples so you can see the bigger picture, and then dive into the details. Each section of the book is paired with exercises to help you practice what you've learned. Although it's tempting to skip the exercises, there's no better way to learn than practicing on real problems.
...
It's possible to divide data analysis into two camps: hypothesis generation (sometimes called exploratory analysis) and hypothesis confirmation (sometimes called confirmatory analysis). The focus of this book is unabashedly on hypothesis generation, or data exploration. Here you'll look deeply at the data and, in combination with your subject knowledge, generate many interesting hypotheses to help explain why the data behaves the way it does. You evaluate these hypotheses informally, using your scepticism to challenge the data in multiple ways.
...
1.  You can only use an observation to confirm a hypothesis once. As soon as
    you use it more than once, you're back to doing exploratory analysis. 
    This means that to do hypothesis confirmation, you need to "preregister" 
...
It's tempting to think about modelling as a tool for hypothesis confirmation, and visualisation as a tool for hypothesis generation. But that's a false dichotomy: models are often used for exploration, and with a little care, you can use visualisation for confirmation. The key difference lies in how often you use each observation: if you use it only once, you're doing hypothesis confirmation; if you use it more than once, you're doing hypothesis exploration.
...
On your own computer, type that line of code in the console, and then press enter to run it. R will download the packages from CRAN and install them on to your computer. If you have problems, make sure that you are connected to the internet, and that <https://cloud.r-project.org/> isn't blocked by your firewall or proxy. 
...
This tells you that tidyverse is loading six packages: ggplot2, tibble, tidyr, readr, purrr, and dplyr. These are considered the __core__ of the tidyverse, because you'll use them in almost every analysis. 
...
* Other R objects (such as data or function arguments) are in a code font
  without parentheses, like `flights` or `x`.
  
* Sometimes, to make it clear which package an object comes from, we'll use
  the package name followed by two colons, like `dplyr::mutate()`, or   
  `nycflights13::flights`. This is also valid R code.

## Getting help and learning more

This book is not an island; there is no single resource that will allow you to master R. As you begin to apply the techniques described in this book to your own data, you will soon find questions that are not answered here. This section describes a few tips on how to get help, and to help you keep learning.

If you get stuck, start with Google. Typically adding "R" to a query is enough to restrict it to relevant results: if the search isn't useful, it often means that there aren't any R-specific results available. Google is particularly useful for error messages. If you get an error message and you have no idea what it means, try googling it! Chances are, someone else has been confused by it in the past, and there will be help somewhere on the web. (If the error message isn't in English, run `Sys.setenv(LANGUAGE = "en")` and re-run the code; you're more likely to find help for English error messages.)
...
1.  **Packages** should be loaded at the top of the script, so it's easy to
    ...
    a bug that's been fixed since you last updated the package. For packages
...
1.  The easiest way to include **data** in a question is to use `dput()` to 
    ...
    dataset in R, you would perform the following steps:
...
This book isn't just the product of Hadley and Garrett, but is the result of many conversations (in person and online) that we've had with many of the people in the R community. There are a few people we'd like to thank in particular, because they have spent many hours answering our dumb questions and helping us to better think about data science:
