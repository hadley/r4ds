...
Even though this pipe has thre steps, it's easy to skim because the verbs come at the start of each line: start with the `flights` data, then filter, then group, then summarize.
...
flights2 <- summarise(flight1,
  .by = tailnum,
  delay = mean(arr_delay, na.rm = TRUE),
  n = n()
)
flights3 <- filter(flights2, n > 5)
```
