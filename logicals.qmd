...
```{r}
flights |> 
  filter(dep_time > 600 & dep_time < 2000 & abs(dep_delay) < 20)
```
...
flights |> 
  mutate(
    ...
    approx_ontime = abs(dep_delay) < 20,
    .keep = "used"
  )
```
...
For example, we could use `all()` and `any()` to find out if every flight was delayed by less than an hour or if any flight was delayed by over 5 hours.
...
flights |> 
  ...
  summarize(
    all_delayed = all(arr_delay < 60, na.rm = TRUE),
...
flights |> 
  group_by(year, month, day) |> 
  summarize(
    all_delayed = mean(arr_delay < 60, na.rm = TRUE),
