...
# Avoid
flights |> summarize(delay = mean(arr_delay, na.rm = TRUE), n = n(),.by=tailnum)
```
...
1.  Restyle the following pipelines following the guidelines above.
    ...
    flights|>filter(dest=="IAH")|>summarize(.by=c(year,month,day),n=n(),delay=mean(arr_delay,na.rm=TRUE))|>filter(n>10)

    flights|>filter(carrier=="UA",dest%in%c("IAH","HOU"),sched_dep_time>0900,sched_arr_time<2000)|>summarize(.by=flight,delay=mean(arr_delay,na.rm=TRUE),cancelled=sum(is.na(arr_delay)),n=n())|>filter(n>10)
    ```
