...
In the @sec-data-visualisation, you learned much more than just how to make scatterplots, bar charts, and boxplots.
...
Let's start with a simple scatterplot of `hwy` versus `displ` of cars in `mpg`.
...
To create this scatterplot we have used the `aes()` function to associate the name of the aesthetic with a variable to display.
Specifically, we mapped `displ` to `x` and `hwy` to `y`.
The syntax highlights a useful insight about `x` and `y`: the x and y locations of a point are themselves aesthetics, visual properties to which you can map variables in order to display information about the data.

<!-- HW: this feels a bit duplicative of the content in the whole game chapter -->
...
You can add a third variable, like `class`, to a scatterplot by mapping it to an aesthetic, like the `color` or the `shape` of your points.
...
In @sec-data-visualisation you learned about faceting with `facet_wrap()`, which splits a plot into subplots that each display one subset of the data based on a categorical variable.
...
<!-- HW: this could maybe be much shorter? -->
...
One layer we have not yet touched on is themes, which will introduce in @sec-themes.
