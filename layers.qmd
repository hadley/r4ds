...
The first argument of `facet_grid()` is also a variable name quoted by `vars()` for rows, and the second argument is for columns.
...
ggplot(mpg, aes(x = displ, y = hwy)) + 
  geom_point() + 
  facet_grid(vars(drv), vars(cyl), scales = "free_y")
```
...
2.  What do the empty cells in the plot above with `facet_grid(vars(drv), vars(cyl))` mean?
