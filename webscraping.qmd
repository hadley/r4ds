...
In most cases, you'll use `html_elements()` and `html_element()` together, typically using `html_elements()` to identity elements that will become observations then using `html_element()` to find elements that will become variables.
Lets see this in action using a simple example.
...
`html_text2()`[^webscraping-5] extracts the plain text contents of an HTML element:

[^webscraping-5]: rvest also provides `html_text()` but you should almost always use `html_text2()` since it does a better job of converting nested HTML to text.
...
If you're lucky you data will be already stored in an HTML table, and it'll be a matter of just reading it from that table.
...
You'll often need to do some experimenting to a selector that is both specific (i.e. it doesn't select things you don't care about) and sensitive (i.e. it does select everything you care about).
...
It doesn't always work, but when it it's magic!
...
Every modern browser comes with some toolkit for developer, but we recommend Chrome, even if it isn't your regular browser: its web developer tools are some of the best, and they're immediately available.
Right click on an element on the page and click `Inspect`.
This will open a expandable view of the complete HTML page, centered on the element that you just clicked.
...
This includes a few empty columns, but overall does a good job of capturing the information from the table, but we need to do some more processing to make it easier to use.
First we'll rename the columns to be easier work with, and remove the extraneous whitespace in the rank and title.
Then we can apply `separate_wider_regex()` (from @sec-extract-variables) to pull out the title, year, and rank into their own variables.
...
Even in this case where most of the data comes from table cells, it's still worth look at the raw HTML.
If you do some, you'll discover that we can add a little extra data by using one of the attributes.
...
From time-to-time, you'll hit a site where your `html_elements()` and friends don't see to return anything like what you see in the browser.
...
It's still possible to scrape these types of sites, but rvest needs to use more expensive process: fully simulating the web browser including running all javascript.
...
It use the [chromote package](https://rstudio.github.io/chromote/index.html) which actually runs chrome browser in the background, and gives you additional tools to interact with the site like a human, typing text and clicking buttons.
