...
This returns a `xml_document`[^webscraping-6] object which you'll then manipulate using rvest functions:
...
The retrieves seven elements matching the seven movies found on that page, suggesting that using `section` as a selector is good.
...
Then we'll remove the new lines and extra spaces then apply `separate_wider_regex()` (from @sec-extract-variables) to pull out the title, year, and rank into their own variables.
