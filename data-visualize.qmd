...
In addition to the tidyverse package we will also use the **palmerpenguins** package, which includes the `penguins` dataset containing body measurements (flipper length body mass, and bill dimensions) for three penguins across in three islands in the Palmer Archipelago.
...
This data frame contains many `r ncol(penguins)` columns.
Another way of viewing the data where we can see all variables and the first few observations from each of the variables is with the `glimpse()` function.
...
Our ultimate goal in this chapter is to recreate the following visualization displaying the relationship between is to create the following visualization of the relationship between bill depths and lengths of these penguins, taking into consideration the species.
...
Then, we need to tell `ggplot()` which variables from this data frame we want to map to visual properties (**aesthetics**) of our plot.
For now, we will only map flipper length to the `x` aesthetic and body mass to the `y` aesthetic in the `mapping` argument, which defines how variables in your dataset are mapped to visual properties of your plot.
...
You'll learn a whole bunch of them throughout the book, particularly in @sec-geoms-stats.
...
Finally, we have something that looks like what we might think of as a "scatter plot".
It doesn't yet match out goal we showed earlier, but using this plot we can start answering the question that motivated our exploration: "What does the relationship between flipper length and body mass look like?" The relationship appears to be positive, fairly linear, and moderately strong.
...
So, next, let's see if adding information on the species of penguins reveal additional insights into this apparent relationship.
We will do this representing species with different colored points.

This means we want to incorporate information from a new variable, `species`, from our dataset into our plot.
Where should `species` go in the ggplot call from earlier?
...
Since this is a new geometric object representing our data, we will do it with a new geom: `geom_smooth()`.
...
We have successfully added a smooth curves, but this plot doesn't look like the plot from @sec-ultmate-goal, which only has one curve for the entire dataset as opposed to three separate curves, one for each of the penguin species.
...
Each geom function in ggplot2 can also take a `mapping` argument, which allows for aesthetic mappings at the *local* level.
...
It's generally not a good idea to represent data using only colors, as people perceive colors differently due to color blindness or other color vision differences.
...
Note that the legend is automatically updated to reflect the different shapes as well.
...
How you visualize the distribution of a single numerical variable depends on the type of variable: categorical or numerical.

A variable is **categorical** if it can only take one of a small set of values.
In R, categorical variables are usually saved as factors (which you will learn more about in @sec-factors) or character vectors (which you will learn more about in @sec-strings).
...
In bar plots where the levels of the discrete variable being plotted are not ordered, like the penguin `species`above, it's often preferable to reorder the bars based on their frequencies.
...
You will learn about many other geoms for visualizing distributions in @sec-geoms-stats.
...
Below are two stacked bar plots, both displaying the relationship between `island` and `species`.
...
The relative frequency plot, created by setting `position = "fill"` in the geom call is more useful for comparing species distributions across islands since it's not affected by the unequal numbers of penguins across the islands.
...
A scatterplot is the most commonly used plot for visualizing the relationship between two variables.
...
You then learned about increasing the complexity and improving the presentation of your plots by adding more layers to it.
