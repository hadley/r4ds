...
When a categorical variable is mapped to an aesthetic element, ggplot2 will automatically assign a unique value of the aesthetic (here a unique color) to each unique level of the variable (each of the three species), a process known as **scaling**.
...
We can also map `species` to both `color` and `fill` aesthetics and using the `alpha` aesthetic to add transparency to the filled density curves.
...
As we saw in our Ultimate Goal, we can incorporate more variables into a plot by mapping them to additional aesthetics.
...
1.  Which variables in `mpg` are categorical in the context of the data?
